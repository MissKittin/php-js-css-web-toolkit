 ...
 * Example usage (add <div id="app"></div> in the <body>):
	...
	// create main window
		...
		main.createLabel('Text label', {'color': '#0000aa'}).createLineBreak(); // the second argument is optional and passes data to the element.style array
...
class wicdPhpGuiWindows
{
	...
	createFormInput(type, name, placeholder=null, defaultValue=null)
	{
		this.#windowContent.appendChild(
			this.#elementsObject
			.	createFormInput(type, name, placeholder, defaultValue)
			.	getContent()
			.	children[0]
		);
...
class wicdPhpGuiWindowsElement
{
	 ...
	 *  constructor accepts associative array only
	 *
	 * Available methods - elements (shared with wicdPhpGuiWindows, [] means optional):
	 ...
	 *  createLabel(string_label, [{'styleParameter': 'style-value'}])
	 *   text inside span
	 *   the second argument passes data to the element.style array
	 *  createLineBreak()
	...
	createLabel(label, style=null)
	{
		...
		if(style !== null)
			for(var [styleParam, styleValue] of Object.entries(style))
				span.style[styleParam]=styleValue;
