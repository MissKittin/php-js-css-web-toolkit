function richTextEditor(boxId, sourceName, params={})
{
	/*
	 ...
	 *  start the editor (3rd arg is optional, note the height):
		richTextEditor('richTextEditorBox', 'richTextEditorOutput', {
	...
	var RTEBox=document.getElementById(boxId);
	...
	RTEButtons[9].addEventListener('click', function(){
		if(typeof params.imgCallback === 'undefined')
			...
			address=params.imgCallback();

		if((address !== '') && (address !== null) && (address !== false))
			RTEditor.execCommand('insertImage', false, address);
	}, true);
