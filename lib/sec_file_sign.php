<?php
	...
	final class file_sign
	{
		 ...
		 *   params:
		 *    'private_key'=>string_path (default: null)
		 ...
		 *      this parameter is required for generation/encryption
		 *      verification/decryption does not require this
		 *    'public_key'=>string_path
		 ...
		 *
		 * Usage:
			$filesign=new file_sign([ // generate/encrypt and verify/decrypt
				'private_key'=>'./private.pem',
				'public_key'=>'./public.pem'
			]);
			...
			$filesign=new file_sign([ // verify/decrypt only
				'private_key'=>'./private.pem',
				'public_key'=>'./public.pem'
			]);
		...
		public static function generate_keys(array $params)
		{
			...
			if(file_put_contents(
				...
				throw new file_sign_exception(
					'Unable to write private key file'
				);

			if(file_put_contents(
				$params['public_key'],
				openssl_pkey_get_details($keys)['key']
			) === false)
				throw new file_sign_exception(
					'Unable to write public key file'
				);
		}
		...
		public function __construct(array $params)
		{
			...
			if(!isset($params['public_key']))
				throw new file_sign_exception(
					'The public_key parameter was not specified for the constructor'
				);

			$this->public_key=realpath(
				$this->public_key
			);

			if($this->public_key === false)
				throw new file_sign_exception(
					'public_key file does not exists'
				);
		...
		public function generate_input_signature(string $data)
		{
			if($this->private_key === null)
				throw new file_sign_exception(
					'The private_key is not set - you can only verify signatures and decrypt data'
				);
		...
		public function verify_input_signature(
			string $data,
			string $signature
		){
		...
		public function generate_file_signature(string $file)
		{
			...
			return $this->generate_input_signature(sha1_file(
				$file,
				true
			));
		}
		public function verify_file_signature(
			string $file,
			string $signature
		){
			if(!file_exists($file))
			...
			return $this->verify_input_signature(
				sha1_file($file, true),
				$signature
			);
		}
