<?php
	...
	namespace Test
	{
		...
		echo ' -> Testing lv_cookie_session_handler (large)';
			...
			echo '  -> testing header_register_callback';
				if(session_status() === PHP_SESSION_ACTIVE)
					echo ' [ OK ]';
				else
				{
					echo ' [FAIL]';
					$errors[]='lv_cookie_session_handler (large) header_register_callback test phase 1';
				}
				ob_end_flush();
				if(session_status() === PHP_SESSION_ACTIVE)
				{
					echo ' [FAIL]'.PHP_EOL;
					$errors[]='lv_cookie_session_handler (large) header_register_callback test phase 2';
					session_write_close();
				}
				else
					echo ' [ OK ]'.PHP_EOL;
		...
		// output buffer closed after lv_cookie_session_handler (large) header_register_callback test phase 1
