<?php
	...
	namespace Test
	{
		...
		echo ' -> Removing temporary files';
			...
			@unlink(__DIR__.'/tmp/ob_cache/ob_cache-1.txt__headers__');
			@unlink(__DIR__.'/tmp/ob_cache/ob_cache-2.txt');
			@unlink(__DIR__.'/tmp/ob_cache/ob_cache-2.txt__headers__');
		echo ' [ OK ]'.PHP_EOL;
		...
		if(getenv('TEST_REDIS') === 'yes')
		{
			...
			if(isset($redis_handle))
			{
				...
				$redis_handle->del('ob_cache_test_cache_1__headers__');
				$redis_handle->del('ob_cache_test_cache_2');
				$redis_handle->del('ob_cache_test_cache_2__headers__');
			}
		}
		...
		if(getenv('TEST_MEMCACHED') === 'yes')
		{
			...
			if(isset($memcached_handle))
			{
				...
				$memcached_handle->delete('ob_cache_test_cache_1__headers__');
				$memcached_handle->delete('ob_cache_test_cache_2');
				$memcached_handle->delete('ob_cache_test_cache_2__headers__');
			}
		}
		...
		echo ' -> Testing ob_file_cache'.PHP_EOL;
			echo '  -> permanent cache';
				ob_start();
				header('HEADER: GOOD VALUE');
				ob_file_cache(__DIR__.'/tmp/ob_cache/ob_cache-1.txt', 0);
				echo 'good value';
				@ob_end_clean();
				@ob_end_clean();

				if(file_get_contents(__DIR__.'/tmp/ob_cache/ob_cache-1.txt__headers__') === '["HEADER: GOOD VALUE"]')
					echo ' [ OK ]';
				else
				{
					echo ' [FAIL]';
					$errors[]='ob_file_cache permanent cache headers failed';
				}
			...
			echo '  -> temporary cache';
				...
				if(file_get_contents(__DIR__.'/tmp/ob_cache/ob_cache-2.txt__headers__') === '["HEADER: GOOD VALUE","HEADER: NEW VALUE"]')
					echo ' [ OK ]';
				else
				{
					echo ' [FAIL]';
					$errors[]='ob_file_cache temporary cache headers failed';
				}
		...
		echo ' -> Testing ob_redis_cache'.PHP_EOL;
			...
			if(isset($redis_handle))
			{
				try {
					...
					if($redis_handle->get('ob_cache_test_cache_1__headers__') === '["HEADER: GOOD VALUE"]')
						echo ' [ OK ]';
					else
					{
						echo ' [FAIL]';
						$errors[]='ob_redis_cache permanent cache headers failed';
					}
				...
				try {
					...
					if($redis_handle->get('ob_cache_test_cache_2__headers__') === '["HEADER: GOOD VALUE","HEADER: NEW VALUE"]')
						echo ' [ OK ]';
					else
					{
						echo ' [FAIL]';
						$errors[]='ob_redis_cache temporary cache headers failed';
					}
					...
					{
						echo ' [FAIL]'.PHP_EOL;
						$errors[]='ob_redis_cache temporary cache failed';
					}
				} catch(Throwable $error) {
					echo ' [FAIL]'.PHP_EOL;
		...
		echo ' -> Testing ob_memcached_cache'.PHP_EOL;
			...
			if(isset($memcached_handle))
			{
				try {
					...
					$memcached_handle->get('ob_cache_test_cache_1__headers__');
					if($memcached_handle->get('ob_cache_test_cache_1__headers__') === '["HEADER: GOOD VALUE"]')
						echo ' [ OK ]';
					else
					{
						echo ' [FAIL]';
						$errors[]='ob_memcached_cache permanent cache headers failed';
					}
				...
				try {
					...
					$memcached_handle->get('ob_cache_test_cache_2__headers__');
					if($memcached_handle->get('ob_cache_test_cache_2__headers__') === '["HEADER: GOOD VALUE","HEADER: NEW VALUE"]')
						echo ' [ OK ]';
					else
					{
						echo ' [FAIL]';
						$errors[]='ob_memcached_cache temporary cache headers failed';
					}
					...
					{
						echo ' [FAIL]'.PHP_EOL;
						$errors[]='ob_memcached_cache temporary cache failed';
					}
				} catch(Throwable $error) {
					echo ' [FAIL]'.PHP_EOL;
